...
let listofNames = []
let fullName = runners.forEach(r => {
  listofNames.push(`${r.first_name} ${r.last_name}`)
})
...
let donations = runners.map(r => r.donation)
let ticketPriceTotal = donations.reduce((total, number) => total + number)
...
// Problem 1
let mailingList = runners.map(r => r.email)
console.log(mailingList)

// Problem 2
let highRollers = runners.filter(r => r.donation >= 250)
console.log(highRollers)

// Problem 3
let fancyList = runners.forEach(r => {
  console.log(`${r.first_name} ${r.last_name} donated $${r.donation}.`)
})
